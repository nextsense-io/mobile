...
import 'package:nextsense_consumer_ui/ui/components/big_text.dart';
...
class SessionControlButton extends StatelessWidget {
  ...
  const SessionControlButton(this.stopSession, {super.key, this.text = 'Stop'});
...
class ProtocolScreen extends HookWidget {
  ...
  ProtocolScreen(this.protocol, {super.key});
    ...
    return PageScaffold(
        ...
        child: Column(
            ...
            children: <Widget>[
              ...
              const Spacer(),
              const WaitWidget(message: "Please wait while the device gets ready to record"),
              const Spacer()
            ]));
  }
...
class CountDownTimer extends StatelessWidget {
  ...
  const CountDownTimer({super.key, required this.duration, required this.reverse, this.secondsElapsed});

  @override
  Widget build(BuildContext context) {
    ProtocolScreenViewModel viewModel = context.watch<ProtocolScreenViewModel>();
    return Padding(padding: const EdgeInsets.only(top: 5), child: CircularCountDownTimer(
      ...
      textStyle:
          const TextStyle(fontSize: 28.0, color: NextSenseColors.purple, fontWeight: FontWeight.w500),
