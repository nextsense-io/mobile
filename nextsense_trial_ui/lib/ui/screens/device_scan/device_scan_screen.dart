...
class DeviceScanScreen extends HookWidget {
    ...
    switch (viewModel.scanningState) {
      ...
      case ScanningState.FINISHED_SCAN:
        return Container(height: 500, child: Column(mainAxisAlignment: MainAxisAlignment.start,
          children: <Widget>[
            Expanded(flex: 20, child: Center(child: UnderlinedTextButton(text: 'Scan again',
      ...
      case ScanningState.CONNECTED:
        return Column(mainAxisAlignment: MainAxisAlignment.start, children: [
          Center(child: Image(image: AssetImage('assets/images/earbuds.png'), width: 180)),
          SizedBox(height: 20),
          SimpleButton(text: MediumText(text: 'Continue', color: NextSenseColors.darkBlue),
              onTap: () => _navigation.canPop() ? _navigation.pop() :
              _navigation.navigateTo(DashboardScreen.id)),
          SizedBox(height: 20),
          SvgPicture.asset('assets/images/circle_checked_blue.svg', semanticsLabel: 'connected',
              height: 60),
          SizedBox(height: 20),
        ]);
      case ScanningState.NOT_FOUND_OR_ERROR:
        return Column(mainAxisAlignment: MainAxisAlignment.start, children: [
          ...
          SizedBox(height: 20),
          MediumText(text: 'NextSense earbuds failed to connect. Please make sure your device is '
              'charged and turned on.', color: NextSenseColors.darkBlue),
          SizedBox(height: 20),
          SimpleButton(text: MediumText(text: 'Try again', color: NextSenseColors.darkBlue),
              onTap: () => viewModel.startScan()),
          SizedBox(height: 20),
          UnderlinedTextButton(
              text: 'Not now',
  ...
  Widget buildBody(DeviceScanScreenViewModel viewModel, BuildContext context) {
    ...
    return viewModel.scanningState == ScanningState.CONNECTING ? connectingBody : scanningBody;
  }
